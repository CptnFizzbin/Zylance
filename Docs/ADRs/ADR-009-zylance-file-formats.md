...
2. Portable (users can backup/restore easily, ideally a single file)
...
- Contains a special `_zylance_` marker table to identify it as a Zylance
  database and prevent accidentally overwriting other SQLite databases

### User Choice

An option in the UI allows users to toggle between encrypted and unencrypted
formats. This gives users control over the security/transparency tradeoff.
...
- **User education**: Users need to understand the security implications of
...
The decision to support both encrypted and plaintext formats is deliberate.
While security experts might argue for mandatory encryption, we believe users
should have control over their data and understand what we're storing.
...
- **Relational database with SQL**: Powerful querying and data modeling
  capabilities
...
- **Well-documented and widely understood**: Extensive documentation and
  community support
- **Good performance for local-first apps**: Optimized for single-user scenarios
- **Strong ecosystem of tools and libraries**: Many tools available for
...
**Encryption approach:**
The encryption will likely use SQLCipher or a similar solution that provides
transparent encryption at the SQLite layer. This means:
...
- Could support encrypted backups to cloud storage
    - will likely leave that to users to implement with their own tools, but we
...
**For future blog post**: Could explore the tension between "security by
default" and "user transparency." Most apps choose one or the other, but Zylance
deliberately supports both. This reflects a philosophy that users should
understand and control what software does with their data, even if it means they
might choose less secure options.
