...
public class MemoryBucketedStorage<TItemId>(uint maxItemsPerBucket = 100) : IBucketedStorage<TItemId>
    where TItemId : notnull
{
    public IBucketManager<TItemId> Buckets { get; } =
        new MemoryBucketManager<TItemId> { MaxItemsPerBucket = maxItemsPerBucket };
